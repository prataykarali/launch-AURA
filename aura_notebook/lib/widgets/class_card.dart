...
class _ClassCardState extends State<ClassCard>
  ...
  // FIX: All pop logic lives here. The sheet receives raw callbacks that do
  // NOT pop — _showOptions is the single authority that closes the sheet and
  // then calls the action. This eliminates any double-pop risk.
  void _showOptions() {
    HapticFeedback.mediumImpact();
    // Capture scaffoldCtx before the async gap.
    final scaffoldCtx = context;

    showModalBottomSheet<void>(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (sheetCtx) => _CardOptionsSheet(
        data: widget.data,
        classCode: _classCode,
        // Each action: pop via sheetCtx (the route owner), then invoke.
        onOpen: () {
        ...
        // The copy button is now handled here too — pass sheetCtx and
        // scaffoldCtx so the sheet never needs to touch outer routes.
        onCopyCode: () {
          Clipboard.setData(ClipboardData(text: _classCode));
          Navigator.of(sheetCtx).pop();
          ScaffoldMessenger.of(scaffoldCtx).showSnackBar(_buildSnack(
            scaffoldCtx,
            'Code "$_classCode" copied!',
          ));
        },
      ),
    );
  ...
  void _shareCode(BuildContext ctx) {
    Clipboard.setData(ClipboardData(text: _classCode));
    if (!mounted) return;
    ScaffoldMessenger.of(ctx).showSnackBar(_buildSnack(
      ctx,
      'Class code "$_classCode" copied to clipboard!',
      duration: const Duration(seconds: 3),
    ));
  }

  SnackBar _buildSnack(BuildContext ctx, String msg,
...
// FIX: The sheet has NO Navigator.pop calls of its own. All callbacks already
// include a pop via sheetCtx (passed from _showOptions). This prevents double-
// pop when the sheet's Builder context and the card's context are different
// levels in the navigator stack.
class _CardOptionsSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      // Class code banner — copy button uses onCopyCode (already has pop)
      Padding(
        padding: const EdgeInsets.fromLTRB(20, 0, 20, 16),
        child: Container(
          ...
          child: Row(children: [
            ...
            GestureDetector(
              onTap: onCopyCode, // single authority — no local pop needed
              child: Container(
      ...
      _SheetAction(Icons.open_in_new_rounded, 'Open Class', data.accent, onOpen),
...
class _SheetAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListTile(
    ...
    title: Text(l,
        style: TextStyle(
            color:
            c == Colors.redAccent ? Colors.redAccent : Colors.white,
            fontSize: 14,
            fontWeight: FontWeight.w600)),
    onTap: onTap,
  );
