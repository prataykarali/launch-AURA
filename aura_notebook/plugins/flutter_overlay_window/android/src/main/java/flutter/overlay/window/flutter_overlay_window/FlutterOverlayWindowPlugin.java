...
public class FlutterOverlayWindowPlugin implements
    ...
    public static boolean isOverlayEngine = false;
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        if (!isOverlayEngine && WindowSetup.messenger == null) {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        if (call.method.equals("checkPermission")) {
            ...
            // PATCHED for Android 14+/15+/16 (Samsung One UI, API 34-36):
            // We deliberately use plain startService() — NOT
            // startForegroundService(). The reason is the circular dependency
            // introduced on Android 15+: a SYSTEM_ALERT_WINDOW app can only
            // call startForeground() AFTER its overlay window is visible, but
            // the overlay window is only created inside onStartCommand() —
            // which runs AFTER onCreate(). If we use startForegroundService(),
            // the system arms a ~5-10s timer expecting startForeground() to be
            // called from onCreate()/onStartCommand(); when the Samsung AMS
            // denies startForeground() (because the window isn't up yet) the
            // timer fires and kills the whole app with
            // ForegroundServiceDidNotStartInTimeException.
            //
            // Using startService() means there is NO timer. The service starts
            // as a background service, adds its overlay window in
            // onStartCommand(), and THEN calls startForeground() via
            // OverlayService.promoteToForeground(). If the promotion is denied,
            // we gracefully continue as a background service — the overlay
            // window is already on screen, so no crash.
            try {
                context.startService(intent);
            } catch (IllegalStateException e) {
                // Background-start restriction on some OEM builds: retry via
                // startForegroundService() as a last resort.
                Log.w("OverlayPlugin", "startService denied, trying startForegroundService: " + e.getMessage());
                try {
